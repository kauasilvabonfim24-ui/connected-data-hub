...
import {
  LayoutDashboard, Calendar, Users, Wallet, Wrench,
  Sparkles, MessageCircle, Megaphone, Bell, Settings, LogOut
} from 'lucide-react'
  ...
  return (
    <aside className="hidden w-64 shrink-0 flex-col border-r border-surface-border bg-white md:flex">
      <div className="flex items-center gap-2.5 px-5 py-5">
        <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-brand text-amber font-display text-base font-bold">
          S
        </div>
        <div>
          <p className="font-display text-sm font-bold leading-none text-ink">Servix IA</p>
          <p className="text-[11px] leading-none text-ink-soft mt-0.5">{empresa?.nome ?? 'Sua empresa'}</p>
        </div>
      </div>
